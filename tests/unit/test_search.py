...
from chronos_mcp.search import SearchOptions, search_events as search_events_func, search_events_ranked, calculate_relevance_score


class TestSearchOptions:
    def test_search_options_defaults(self):
        """Test SearchOptions with default values"""
        opts = SearchOptions(query="meeting")
...
class TestSearchEvents:
    ...
    def test_basic_contains_search(self):
        ...
        opts = SearchOptions(query="team", case_sensitive=False)
        
        results = search_events_func(events, opts)
        assert len(results) == 3  # Team Meeting, team sync, Team lunch
        
        # Case sensitive
        opts_case = SearchOptions(query="team", case_sensitive=True)
    ...
    def test_match_type_search(self):
        ...
        # Starts with
        opts = SearchOptions(query="team", match_type='starts_with')
        results = search_events_func(events, opts)
        assert len(results) == 2  # Team Meeting, Team lunch
        
        # Ends with
        opts = SearchOptions(query="call", match_type='ends_with')
        results = search_events_func(events, opts)
        assert len(results) == 1  # Client Call
        
        # Exact match
        opts = SearchOptions(query="Lunch Break", match_type='exact')
        results = search_events_func(events, opts)
    ...
    def test_regex_search(self):
        ...
        opts = SearchOptions(query=r"Room\s+[A-Z]", use_regex=True)
        results = search_events_func(events, opts)
        assert len(results) == 2  # Conference Room A, Meeting Room B
        
        # Match events with numbers
        events[0]['summary'] = 'Team Meeting #123'
        opts = SearchOptions(query=r'#\d+', use_regex=True)
        results = search_events_func(events, opts)
        assert len(results) == 1
    ...
    def test_max_results_limiting(self):
        ...
        opts = SearchOptions(query="e", max_results=2)  # Matches all events
        results = search_events_func(events, opts)
...
class TestRelevanceScoring:
    def test_field_weight_scoring(self):
        ...
        opts = SearchOptions(query="important")
    ...
    def test_position_scoring(self):
        ...
        # Match at beginning
        event_start = {'summary': 'Meeting with client', 'description': '', 'location': ''}
        
        # Match at end
        event_end = {'summary': 'Client discussion meeting', 'description': '', 'location': ''}
    ...
    def test_search_events_ranked(self):
        ...
        events = [
            {'uid': '1', 'summary': 'Important Team Meeting', 'description': '', 'location': ''},
            {'uid': '2', 'summary': 'Meeting', 'description': 'Important topics', 'location': ''},
            {'uid': '3', 'summary': 'Lunch', 'description': '', 'location': 'Meeting Room'},
            {'uid': '4', 'summary': 'Team Meeting Tomorrow', 'description': '', 'location': ''}
        ]
