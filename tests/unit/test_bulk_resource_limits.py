...
class TestBulkResourceLimits:
    ...
    def test_default_max_parallel_is_reasonable(self):
        ...
        assert options.max_parallel is not None, (
            "max_parallel should have default value"
        )
        assert options.max_parallel <= 20, (
            f"Default max_parallel too high: {options.max_parallel}"
        )
        assert options.max_parallel >= 5, (
            f"Default max_parallel too low: {options.max_parallel}"
        )
