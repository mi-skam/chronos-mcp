...
class TestUrlValidationSecurity:
    ...
    def test_malicious_url_schemes_rejected(self):
        ...
        for url in malicious_urls:
            assert not validator.PATTERNS["url"].match(url), f"Malicious URL should be rejected: {url}"
    ...
    def test_url_injection_attempts_rejected(self):
        ...
        for url in injection_urls:
            assert not validator.PATTERNS["url"].match(url), f"Dangerous URL should be rejected: {url}"
    ...
    def test_localhost_and_private_ips_allowed(self):
        ...
        for url in local_urls:
            assert validator.PATTERNS["url"].match(url), f"Local/private URL should be allowed: {url}"
    ...
    def test_empty_and_malformed_urls(self):
        ...
        for url in malformed_urls:
            assert not validator.PATTERNS["url"].match(url), f"Malformed URL should be rejected: {url}"
    ...
    def test_unicode_domains_handled(self):
        ...
        assert validator.PATTERNS["url"].match(unicode_domains[0]), "Punycode domain should be allowed"
        assert not validator.PATTERNS["url"].match(unicode_domains[1]), "Direct Unicode should be rejected (security feature)"
    ...
    def test_case_sensitivity(self):
        ...
        assert not validator.PATTERNS["url"].match(case_variants[0]), "Uppercase HTTPS should be rejected"
        assert not validator.PATTERNS["url"].match(case_variants[1]), "Mixed case Https should be rejected"
        assert not validator.PATTERNS["url"].match(case_variants[2]), "Mixed case HTTPs should be rejected"
