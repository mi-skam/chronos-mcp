...
import pytz
from icalendar import Event as iEvent, Calendar as iCalendar, Alarm
import uuid

from chronos_mcp.events import EventManager
from chronos_mcp.models import Event, Attendee, Alarm as AlarmModel
...
class TestEventManager:
    ...
    def test_create_event_success(self, mock_uuid, mock_calendar_manager, mock_calendar, sample_event_data):
    ...
    def test_create_event_with_attendees(self, mock_calendar_manager, mock_calendar, sample_event_data):
        """Test creating event with attendees"""
        mock_calendar_manager.get_calendar.return_value = mock_calendar
        
        attendees = [
            {"email": "user1@example.com", "name": "User One", "role": "REQ-PARTICIPANT"},
            {"email": "user2@example.com", "name": "User Two", "role": "OPT-PARTICIPANT", "rsvp": False}
        ]
    ...
    def test_create_event_with_recurrence(self, mock_calendar_manager, mock_calendar, sample_event_data):
