    ...
    def validate_rrule(cls, rrule_string: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if freq_str not in cls.ALLOWED_FREQUENCIES:
                return False, f"Frequency {freq_str} not allowed. Use: {', '.join(cls.ALLOWED_FREQUENCIES.keys())}"
            ...
            if not has_count and not has_until:
                return False, "RRULE must have COUNT or UNTIL to prevent infinite recurrence"
                ...
                if until_value:
                    try:
                        ...
                        if until_dt > max_future:
                            return False, f"UNTIL date cannot be more than {MAX_YEARS_AHEAD} years in the future"
                            
                    except ValueError:
                        return False, "UNTIL must be a valid date in YYYYMMDD or YYYYMMDDTHHMMSSZ format"
