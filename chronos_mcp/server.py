...
async def create_recurring_event(
    ...
    recurrence_rule: str = Field(
        ...,
        description="RRULE for recurring events (e.g., 'FREQ=WEEKLY;BYDAY=MO,WE,FR;COUNT=10')",
    ),
    description: Optional[str] = Field(None, description="Event description"),
    location: Optional[str] = Field(None, description="Event location"),
    alarm_minutes: Optional[str] = Field(
...
async def search_events(
    ...
    fields: Optional[List[str]] = Field(
        None,
        description="Fields to search in (default: summary, description, location)",
    ),
    case_sensitive: bool = Field(False, description="Case-sensitive search"),
    date_start: Optional[str] = Field(
        None, description="Start date for range filter (ISO format)"
    ),
    date_end: Optional[str] = Field(
        None, description="End date for range filter (ISO format)"
    ),
    calendar_uid: Optional[str] = Field(
        None, description="Specific calendar to search (searches all if not specified)"
    ),
                ...
                for event in events:
                    ...
                    for field in fields:
                        ...
                        if value is None:
                            value = ""
                        check_value = (
                            str(value).lower() if not case_sensitive else str(value)
                        )
...
async def bulk_create_events(
    ...
    validate_before_execute: bool = Field(
        True, description="Validate all events before execution"
    ),
    account: Optional[str] = Field(
        None, description="Account alias (uses default if not specified)"
    ),
    ...
    try:
        ...
        if mode not in ["continue", "fail_fast"]:
            raise ValueError(
                f"Invalid mode '{mode}'. Must be one of: continue, fail_fast"
            )
        ...
        # Process events
        results = {"total": len(events), "succeeded": 0, "failed": 0, "details": []}
        ...
        for i, event_data in enumerate(events):
            try:
                ...
                alarm_mins = None
                if (
                    "alarm_minutes" in event_data
                    and event_data["alarm_minutes"] is not None
                ):
                    try:
                        ...
                        if not -10080 <= alarm_mins <= 10080:
                            raise ValueError(
                                f"alarm_minutes must be between -10080 and 10080, got {alarm_mins}"
                            )
...
async def bulk_delete_events(
    ...
    try:
        ...
        results = {"total": len(event_uids), "succeeded": 0, "failed": 0, "details": []}
        ...
        for uid in event_uids:
            try:
                ...
                results["succeeded"] += 1
                results["details"].append({"uid": uid, "success": True})
...
async def bulk_create_tasks(
    ...
    tasks: List[Dict[str, Any]] = Field(
        ..., description="List of task data dictionaries"
    ),
    mode: str = Field(
        "continue", description="Operation mode: continue, fail_fast, or atomic"
    ),
    max_parallel: int = Field(5, description="Maximum parallel operations"),
    validate_before_execute: bool = Field(
        True, description="Validate all tasks before execution"
    ),
    dry_run: bool = Field(
...
async def bulk_delete_tasks(
    ...
    try:
        ...
        options = BulkOptions(
            mode=bulk_mode, max_parallel=max_parallel, dry_run=dry_run
        )
...
async def list_tasks(
    ...
    try:
        tasks = task_manager.list_tasks(
            calendar_uid=calendar_uid, account_alias=account, request_id=request_id
        )
...
async def bulk_create_journals(
    ...
    journals: List[Dict[str, Any]] = Field(
        ..., description="List of journal data dictionaries"
    ),
    mode: str = Field(
        "continue", description="Operation mode: continue, fail_fast, or atomic"
    ),
    max_parallel: int = Field(5, description="Maximum parallel operations"),
    validate_before_execute: bool = Field(
        True, description="Validate all journals before execution"
    ),
    dry_run: bool = Field(
        False, description="Simulate the operation without making changes"
    ),
    ...
    try:
        ...
        # Execute bulk operation
        result = bulk_manager.bulk_create_journals(
            calendar_uid, journals, options, account
        )
...
async def create_journal(
    ...
    dtstart: Optional[str] = Field(
        None, description="Journal entry date/time (ISO format)"
    ),
    related_to: Optional[List[str]] = Field(
        None, description="List of related component UIDs"
    ),
