...
class TaskManager:
    ...
    def create_task(self, 
                   ...
                   request_id: Optional[str] = None) -> Optional[Task]:
        ...
        try:
            ...
            if hasattr(calendar, 'save_todo'):
                logger.debug(f"Using calendar.save_todo() for optimized task creation", extra={"request_id": request_id})
                try:
                    caldav_task = calendar.save_todo(ical_data)
                except Exception as e:
                    logger.warning(f"calendar.save_todo() failed: {e}, falling back to save_event()", extra={"request_id": request_id})
                    caldav_task = calendar.save_event(ical_data)
            else:
                logger.debug(f"Server doesn't support calendar.save_todo(), using calendar.save_event()", extra={"request_id": request_id})
                caldav_task = calendar.save_event(ical_data)
            ...
            logger.error(f"Authorization error creating task '{summary}': {e}", extra={"request_id": request_id})
            raise EventCreationError(summary, "Authorization failed", request_id=request_id)
        except Exception as e:
            logger.error(f"Error creating task '{summary}': {e}", extra={"request_id": request_id})
    ...
    def get_task(self, 
        ...
        try:
            ...
            try:
                ...
                for todo in todos:
                    if task_uid in todo.data:
                        task_data = self._parse_caldav_task(todo, calendar_uid, account_alias)
            ...
            logger.error(f"Error getting task '{task_uid}': {e}", extra={"request_id": request_id})
            raise ChronosError(f"Failed to get task: {str(e)}", request_id=request_id)

    def list_tasks(self, 
                  calendar_uid: str,
                  account_alias: Optional[str] = None,
                  request_id: Optional[str] = None) -> List[Task]:
        ...
        try:
            ...
            if hasattr(calendar, 'todos'):
                try:
                    logger.debug(f"Using calendar.todos() for server-side filtering", extra={"request_id": request_id})
                    todos = calendar.todos()
                    
                    for caldav_todo in todos:
                        task_data = self._parse_caldav_task(caldav_todo, calendar_uid, account_alias)
                        if task_data:
                            tasks.append(task_data)
                            
                except Exception as e:
                    logger.warning(f"calendar.todos() failed: {e}, falling back to calendar.events()", extra={"request_id": request_id})
                    # Fall through to fallback method
                    raise
            else:
                # Fallback method for servers without todos() support
                logger.debug(f"Server doesn't support calendar.todos(), using calendar.events() with client-side filtering", extra={"request_id": request_id})
            ...
            if hasattr(calendar, 'todos'):
                try:
                    ...
                    logger.error(f"Error listing tasks (both methods failed): {fallback_error}", extra={"request_id": request_id})
            else:
    ...
    def update_task(self,
        ...
        try:
            ...
            logger.error(f"Error updating task '{task_uid}': {e}", extra={"request_id": request_id})
            raise EventCreationError(
                task_uid,
    ...
    def delete_task(self, 
        ...
        try:
                    ...
                    for component in ical.walk():
                        if component.name == "VTODO":
                            if str(component.get('uid', '')) == task_uid:
                                todo.delete()
                                logger.info(f"Deleted task '{task_uid}'", extra={"request_id": request_id})
                                return True
            except Exception as e:
            ...
            logger.error(f"Authorization error deleting task '{task_uid}': {e}", extra={"request_id": request_id})
            raise EventDeletionError(task_uid, "Authorization failed", request_id=request_id)
        except Exception as e:
            logger.error(f"Error deleting task '{task_uid}': {e}", extra={"request_id": request_id})
            raise EventDeletionError(task_uid, str(e), request_id=request_id)

    def _parse_caldav_task(self, caldav_event: CalDAVEvent, calendar_uid: str, account_alias: Optional[str]) -> Optional[Task]:
