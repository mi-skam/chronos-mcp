...
class CalendarManager:
    ...
    def list_calendars(self, account_alias: Optional[str] = None, request_id: Optional[str] = None) -> List[Calendar]:
        ...
        try:
            for cal in principal.calendars():
                ...
                cal_info = Calendar(
                    uid=str(cal.url).split('/')[-2] if str(cal.url).endswith('/') else str(cal.url).split('/')[-1],
    ...
    def create_calendar(self, name: str, description: Optional[str] = None, 
                       color: Optional[str] = None, account_alias: Optional[str] = None,
                       request_id: Optional[str] = None) -> Optional[Calendar]:
        ...
        try:
            ...
            logger.error(f"Authorization error creating calendar '{name}': {e}", extra={"request_id": request_id})
            raise CalendarCreationError(name, "Authorization failed", request_id=request_id)
        except Exception as e:
            logger.error(f"Error creating calendar '{name}': {e}", extra={"request_id": request_id})
            raise CalendarCreationError(name, str(e), request_id=request_id)

            
    def delete_calendar(self, calendar_uid: str, account_alias: Optional[str] = None,
        ...
        try:
            ...
            for cal in principal.calendars():
                ...
                if cal_id == calendar_uid:
                    cal.delete()
                    logger.info(f"Deleted calendar '{calendar_uid}'", extra={"request_id": request_id})
                    return True
            ...
            logger.error(f"Authorization error deleting calendar '{calendar_uid}': {e}", extra={"request_id": request_id})
            raise CalendarDeletionError(calendar_uid, "Authorization failed", request_id=request_id)
        except Exception as e:
            logger.error(f"Error deleting calendar '{calendar_uid}': {e}", extra={"request_id": request_id})
    ...
    def get_calendar(self, calendar_uid: str, account_alias: Optional[str] = None,
                    request_id: Optional[str] = None) -> Optional[CalDAVCalendar]:
