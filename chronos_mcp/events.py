...
from .utils import parse_datetime, datetime_to_ical, ical_to_datetime, validate_rrule
...
class EventManager:
        ...
        try:
            ...
            if attendees:
                for att in attendees:
                    ...
                    event.add('attendee', attendee_str, parameters={
            ...
            logger.error(f"Authorization error creating event '{summary}': {e}", extra={"request_id": request_id})
            raise EventCreationError(summary, "Authorization failed", request_id=request_id)
        except Exception as e:
            logger.error(f"Error creating event '{summary}': {e}", extra={"request_id": request_id})
    ...
    def get_events_range(self, 
                        ...
                        end_date: datetime,
                        account_alias: Optional[str] = None,
                        request_id: Optional[str] = None) -> List[Event]:
    ...
    def _parse_caldav_event(self, caldav_event: CalDAVEvent, calendar_uid: str, account_alias: Optional[str]) -> Optional[Event]:
            ...
            for component in ical.walk():
                if component.name == "VEVENT":
                    ...
                    if dtstart and dtend:
                        ...
                        elif (start_dt.hour == 0 and start_dt.minute == 0 and start_dt.second == 0 and
                              end_dt.hour == 0 and end_dt.minute == 0 and end_dt.second == 0 and
    ...
    def delete_event(self, calendar_uid: str, event_uid: str, account_alias: Optional[str] = None,
        ...
        try:
                ...
                for component in ical.walk():
                    if component.name == "VEVENT":
                        if str(component.get('uid', '')) == event_uid:
                            event.delete()
                            logger.info(f"Deleted event '{event_uid}'", extra={"request_id": request_id})
                            return True
            ...
            logger.error(f"Authorization error deleting event '{event_uid}': {e}", extra={"request_id": request_id})
    ...
    def update_event(self,
        ...
        try:
            ...
            if attendees is not None:
                ...
                for att in attendees:
                    attendee_str = f"mailto:{att['email']}"
                    existing_event.add('attendee', attendee_str, parameters={
                        'CN': att.get('name', att['email']),
                        'ROLE': att.get('role', 'REQ-PARTICIPANT'),
                        'PARTSTAT': att.get('status', 'NEEDS-ACTION'),
                        'RSVP': 'TRUE' if att.get('rsvp', True) else 'FALSE'
            ...
            logger.error(f"Error updating event '{event_uid}': {e}", extra={"request_id": request_id})
            raise EventCreationError(
                event_uid,
